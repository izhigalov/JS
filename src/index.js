...
function findAllPSiblings(where) {
    var where = document.innerHTML('<div></div><p></p><a></a><span></span><p></p>');
    var result = [];
    for (var i = 0; i <where.childElements.lenght; i++){
        if (where.childElements[i].nextElementSubling('p')){
            result.push(where.childElements[i]);
        }
...
function deleteTextNodes(where) {
    ...
    var list = where.childNodes;
    var result = [];
    for (var i = 0; i < list.length; i++) {
        result.push(list.childElements[i]);
    }
    for (i = 0; i < result.length; i++) {
        if (result[i].nodeType == 3) {
            where.removeChild(result[i]);
        }
    }
}   
console.log(deleteTextNodes(where));
...
function deleteTextNodesRecursive(where) {
    ...
    for (i = 0; i < result.length; i++) {
        ...
        if (result[i].childNodes.lenght > 0) {
            deleteTextNodesRecursive(result[i]);
        }
    }
}   
console.log(deleteTextNodes(where));
